config=
	...
	dimensions: 
		imd:
			...
			inputformat: (data) -> return ((f) -> Math.round(f*100)/100)
			axisformat: (data) -> Math.round
		...
		price:
			...
			renderForm: (data,container) -> $(container).html($("#templates .price").html())
			format: (data) ->
				if data.p.type=='sale'
					return (v) -> "£"+d3.format(",.0f")(v/1000)+"k"
				else
					return (v) -> "£"+d3.format(",.0f")(v)+" per week"
			inputformat: (data) -> return Math.round
			axisformat: (data) ->
				if data.p.type=='sale'
					return (v) -> "£"+d3.format(",.0f")(v/1000)+"k"
				else
